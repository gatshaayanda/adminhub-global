...
test('homepage proof combines product truth, anonymous traffic and restrained independent reputation support', () => {
  ...
  assert.match(proofRail, /Active players/);
  assert.match(proofRail, /Reviews completed/);
  assert.match(proofRail, /Players served/);
  assert.match(proofRail, /Reviews forming now/);
  assert.match(proofRail, /retention Review records/);
  assert.match(proofRail, /LAST 30 DAYS · VERCEL WEB ANALYTICS/);
  assert.match(proofRail, /Anonymous aggregated site activity — not player accounts/);
  assert.match(proofRail, /Read independent reviews on Trustpilot/);
