  ...
  return current.flatMap((standing) => {
    ...
    return [{
      ...
      body: podium
        ...
        : `#${old.rank} → #${standing.rank}. ${improved ? "You moved up the official field." : "A newer completed Desk changed the official field around you."}`,
...
export function deriveProvisionalCards(
  ...
  return boards.flatMap((board) => {
    ...
    return [{
      ...
      title: `${board.title}${board.scopeLabel ? ` · ${board.scopeLabel}` : ""}`,
      body: `PROVISIONAL · Your forming episode would place #${item.rank} of ${board.entries.length} on the current field if it closed with these supported facts. Official standing changes only after the completed Desk publishes.`,
      categoryId: board.categoryId,
      pool: board.scopeLabel,
      finality: "provisional",
    } satisfies PlayerPulseCard];
...
export function nominateShareMoments(
  ...
  for (const pool of desk.pools) {
    ...
    candidates.push({
      ...
      supportingFact: `${pool.games} ${normalized} games in the completed seven-day Desk produced a recorded ${delta > 0 ? "+" : ""}${delta} rating movement.`,
  ...
  if (desk.longestWinStreak >= 3) {
    candidates.push({
      ...
      supportingFact: `${desk.longestWinStreak} consecutive wins were recorded inside the completed seven-day Desk.`,
  ...
  if ((desk.checkmateWins ?? 0) >= 3) {
    candidates.push({
      ...
      supportingFact: `${desk.checkmateWins} wins in the completed Desk ended by checkmate.`,
  ...
  for (const standing of standings.filter((item) => item.rank <= 3)) {
    candidates.push({
      ...
      supportingFact: `${standing.valueLabel} placed this completed Desk #${standing.rank} of ${standing.denominator} in the current official comparison field.`,
