...
export default function PlayerProfileNotifications({
  ...
  async function connectGoogle() {
    ...
    try {
      ...
      setGoogleMessage(body.result?.chessComOwnershipVerified
        ? "Google return access connected. Chess.com ownership is already verified separately."
        : "Google return access connected. Your Chess.com ownership status did not change.");
      await onSaved();
    } catch (reason) {
  ...
  return <section className="player-profile-section">
    ...
    <div className="profile-settings-grid">
      <article className="profile-settings-card"><h3>Account</h3><dl><div><dt>Chess.com username</dt><dd>{account.chessCom.canonicalUsername}</dd></div><div><dt>Founding Access</dt><dd>{account.accessTier === "founding_beta" && account.accessStatus === "active" ? "Active" : account.accessStatus}</dd></div><div><dt>Chess.com ownership</dt><dd>{account.identityStatus === "oauth_verified" ? "Verified with Chess.com" : account.identityStatus === "founder_reviewed" ? "Founder reviewed" : "Unverified / provisional"}</dd></div></dl></article>

      <article className="profile-settings-card"><h3>Google Access</h3><div className="profile-contact-status"><span>BoardSignal return key</span><strong>{account.googleAccessConnectedAt ? "Connected" : "Not connected"}</strong></div><div className="required-participation-row"><KeyRound size={17}/><div><strong>Easy secure return</strong><p>Google can bring you back to this exact BoardSignal UID. It does not prove ownership of the Chess.com profile, merge accounts, or change public identity.</p></div></div>{account.identityStatus === "provisional" ? <p className="profile-helper">Your Chess.com ownership remains provisional even after Google is connected.</p> : null}{account.googleAccessConnectedAt ? <p className="profile-helper">Connected {new Date(account.googleAccessConnectedAt).toLocaleDateString()}. Existing Preview, magic access, fallback access and current sessions remain available.</p> : <button className="button button-outline" type="button" onClick={() => void connectGoogle()} disabled={googleBusy || !connectivity.online}>{googleBusy ? <><LoaderCircle className="button-spinner" size={14}/> Connecting</> : "CONNECT GOOGLE"}</button>}{googleError ? <p className="form-error" role="alert">{googleError}</p> : null}{googleMessage ? <p className="form-success" role="status">{googleMessage}</p> : null}</article>
