...
export default function PlayerProfileNotifications({
  ...
  return <section className="player-profile-section">
    ...
    <div className="profile-settings-grid">
      ...
      <article className="profile-settings-card"><h3>Email alerts</h3>{notificationToggle("deskReady", "Review Ready")}{notificationToggle("episodeProgress", "This Week Progress")}{notificationToggle("blueReminder", "Focus Next Reminder")}{notificationToggle("universeAchievement", "Universe Highlight")}{notificationToggle("founderUpdates", "Founder Updates")}<div className="profile-email-alerts"><p className="kicker">EMAIL ALERTS</p><label className="profile-toggle"><span><strong>Email me important BoardSignal updates</strong><small>{emailContactReady ? "Review Ready, major BoardSignal updates, feedback requests and other important eligible messages only." : "Choose Email as your preferred contact, enter a valid address and explicitly keep BoardSignal contact consent enabled first."}</small></span><input type="checkbox" checked={emailContactReady && notifications.email} disabled={!emailContactReady} onChange={(event) => setNotifications((value) => ({ ...value, email: event.target.checked }))} /></label></div></article>

      <article className="profile-settings-card"><h3>Browser alerts</h3><BrowserPushControl idToken={token} onChanged={onSaved} /></article>

      <article className="profile-settings-card"><h3>Community</h3><div className="required-participation-row"><ShieldCheck size={17}/><div><strong>Public highlights</strong><p>Positive or neutral public coverage remains subject to BoardSignal identity and public-safety gates. Google Access never unlocks public Chess.com identity by itself.</p></div></div><div className="required-participation-row"><ShieldCheck size={17}/><div><strong>BoardSignal Discord</strong><p>Optional community and founder contact. Discord is never required for access and is not identity proof.</p></div></div><a className="button button-outline" href={BOARDSIGNAL_SUPPORT_DISCORD_URL} target="_blank" rel="noreferrer noopener">JOIN THE BOARDSIGNAL DISCORD</a></article>
