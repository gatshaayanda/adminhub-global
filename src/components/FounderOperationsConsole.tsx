      ...
      {shown.length ? <div className="founder-ops-table" role="table" aria-label="Live player operations">
        ...
        {shown.map((row) => {
          ...
          <div className="founder-ops-main-row">
            ...
            <div data-label="HISTORY" className="founder-history-cell"><strong>{history.primary}</strong><small>{history.secondary}</small><small><b>LATEST REPORT</b> · {row.latestReportPeriod?.outcome === "review" ? "REVIEW" : row.latestReportPeriod?.outcome === "no_activity" ? "NO ACTIVITY" : "SYNCING"} · {row.latestReportPeriod?.periodLabel ?? "No completed report period"}{row.historyCoverage ? ` · ${row.historyCoverage.evaluatedCount}/${row.historyCoverage.totalCount} evaluated` : ""}</small><small><b>OFFICIAL CHESS STATE</b> · {row.officialChessStatePeriod?.periodLabel ?? "No game-bearing Review"}</small></div>
          ...
          <details className="founder-ops-details"><summary>Details</summary><div className="founder-ops-detail-grid">
            <dl><div><dt>Stable Chess.com ID</dt><dd>{row.playerId ?? "Not yet recorded"}</dd></div><div><dt>Identity</dt><dd>{row.identityStatus ?? "Not recorded"}{row.identityReviewStatus ? ` · ${row.identityReviewStatus}` : ""}</dd></div><div><dt>Account</dt><dd>{row.accountStatus ?? "Pending request"}</dd></div><div><dt>Public highlights</dt><dd>{row.publicHighlightsStatus ?? "Not available yet"}</dd></div><div><dt>Preferred contact</dt><dd>{row.preferredContactMethod && row.preferredContactValue ? `${row.preferredContactMethod}: ${row.preferredContactValue}` : "Not confirmed"}</dd></div><div><dt>Latest completed report period</dt><dd>{row.latestReportPeriod ? `${row.latestReportPeriod.periodLabel} · ${row.latestReportPeriod.outcome === "review" ? "REVIEW" : row.latestReportPeriod.outcome === "no_activity" ? "NO ACTIVITY" : "SYNCING"}` : "None"}</dd></div><div><dt>Official chess state period</dt><dd>{row.officialChessStatePeriod?.periodLabel ?? "No game-bearing Review"}</dd></div><div><dt>Current forming</dt><dd>{row.forming ? "Yes" : "No"}</dd></div><div><dt>{row.reviewDateLabel}</dt><dd>{row.nextDeskDueAt ?? "Not recorded"}</dd></div><div><dt>History coverage</dt><dd>{history.primary} · {history.secondary}</dd></div><div><dt>Unread replies</dt><dd>{row.unreadReplies}</dd></div><div><dt>Last Founder contact</dt><dd>{row.lastContactedAt ? `${new Date(row.lastContactedAt).toLocaleString()}${row.lastContactMethod ? ` · ${row.lastContactMethod}` : ""}` : "Not recorded"}</dd></div><div><dt>Follow-up reason</dt><dd>{row.attentionReasons.join(" · ") || "No operational attention"}</dd></div><div><dt>Access status</dt><dd>{row.accessStatus ?? "Request pending"}</dd></div></dl>
