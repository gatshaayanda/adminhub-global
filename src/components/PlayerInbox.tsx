...
export default function PlayerInbox({ token, onUnreadChange }: { token: string; onUnreadChange?: (count: number) => void }) {
  ...
  return (
    <section className="player-inbox-section">
      <div className="room-section-heading"><div><p className="kicker">INBOX</p><h2>BoardSignal messages</h2><p>Review updates, Universe achievements, founder notes and beta feedback requests stay inside your private My BoardSignal.</p></div><button type="button" className="button button-quiet" onClick={load} disabled={networkRequired || loading}>Refresh</button></div>
