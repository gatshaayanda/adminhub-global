...
export default function OfflinePlayerRoom({ uid: suppliedUid, initialSnapshot, embedded = false }: { uid?: string; initialSnapshot?: OfflinePlayerRoomSnapshot; embedded?: boolean }) {
  ...
  useEffect(() => {
    if (snapshot?.desks[0] && !selectedDeskKey) setSelectedDeskKey(snapshot.desks[0].summary.deskKey);
  }, [selectedDeskKey, snapshot]);
  ...
  if (!uid) return <div id="main" className="container offline-player-room-empty"><WifiOff size={24}/><p className="kicker">BOARDSIGNAL OFFLINE</p><h1>Your saved BoardSignal needs the same signed-in account.</h1><p>Signing in needs a connection. Reconnect, sign in once, and BoardSignal can keep your saved Reviews available on this device.</p></div>;
  if (!snapshot || snapshot.uid !== uid) return <div id="main" className="container offline-player-room-empty"><WifiOff size={24}/><p className="kicker">BOARDSIGNAL OFFLINE</p><h1>No BoardSignal has been saved for this account on this device yet.</h1><p>Reconnect and open My BoardSignal once. BoardSignal will keep a bounded saved copy for this account on this device.</p></div>;
  ...
  return <div id="main" className={`offline-player-room player-room-authenticated ${embedded ? "is-embedded" : ""}`}>
    <header className="container offline-room-identity">
      <div className="universal-avatar">{snapshot.canonicalUsername.slice(0,2).toUpperCase()}</div>
      <div><span>MY BOARDSIGNAL · SAVED</span><h1>{snapshot.canonicalUsername}</h1><p><WifiOff size={14}/> You're offline. Showing your saved BoardSignal from {savedLabel(snapshot.lastSyncedAt)}.</p></div>
    </header>
    <div className="container offline-room-truth"><strong>SAVED</strong><span>New Chess.com games, Universe movement, messages and account changes are not included after {savedLabel(snapshot.lastSyncedAt)}.</span></div>
    <nav className="container offline-room-tabs" aria-label="Saved My BoardSignal">
      {(["desk","progress","universe","friends"] as OfflineTab[]).map((item) => <button type="button" key={item} className={tab===item?"active":""} onClick={() => setTab(item)} aria-current={tab===item?"page":undefined}>{item === "desk" ? "Review" : item === "progress" ? "Progress" : item === "universe" ? "Universe" : "Friends"}</button>)}
    </nav>
    ...
    {tab === "progress" ? <div className="container player-room-memory"><section className="my-progress-section">
      <div className="universal-section-heading"><span><TrendingUp size={16}/></span><div><p className="kicker">PROGRESS · SAVED</p><h2>Your recent Reviews.</h2><p>Saved {savedLabel(snapshot.lastSyncedAt)}. This view does not calculate new conclusions from games BoardSignal has not synchronized.</p></div></div>
      {snapshot.desks.length <= 1 ? <div className="progress-start-state"><h3>Your first Review is your baseline.</h3><p>There is nothing meaningful to compare yet. After another completed Review, Progress can show what changed and what repeated.</p><small>Your first Review still counts; it is the starting point.</small></div> : null}
      <div className="desk-sequence">{[...snapshot.desks].reverse().map((item,index) => <article key={item.summary.deskKey}><span>SAVED REVIEW {index+1}</span><strong>{item.summary.periodLabel}</strong><p>{item.summary.games} games · {item.summary.scorePct.toFixed(1)}%</p></article>)}</div>
      {snapshot.progress.map((series) => <div className="pool-progress" key={series.pool}><h3>{series.pool} progress</h3><div>{series.points.length ? <article><span>Saved score movement</span><strong>{series.points.map((point) => `${point.scorePct}%`).join(" → ")}</strong></article> : null}{series.points.filter((point)=>point.ratingDelta!==undefined).length ? <article><span>Saved rating movement</span><strong>{series.points.filter((point)=>point.ratingDelta!==undefined).map((point)=>`${Number(point.ratingDelta)>=0?"+":""}${point.ratingDelta}`).join(" → ")}</strong></article> : null}</div></div>)}
      <div className="personal-record-strip"><BarChart3 size={18}/><div><span>Saved personal record</span><strong>{snapshot.personalRecords.personalBestWinRun} straight wins</strong></div><div><span>Reviews completed</span><strong>{snapshot.personalRecords.desksCompleted}</strong></div></div>
      <PlayerReviewNotesTimeline token="" online={false} journal={snapshot.reviewJournal ?? { version: 1, notes: [] }} onJournalChanged={() => undefined} />
    </section></div> : null}

    {tab === "universe" ? <div className="container player-room-memory"><section className="pulse-universe-block">
      <div className="pulse-block-heading"><div><p className="kicker">UNIVERSE · SAVED</p><h2>Last synchronized {savedLabel(snapshot.lastSyncedAt)}</h2></div><span>Not live</span></div>
      {snapshot.pulse ? <>
        {(snapshot.pulse.sinceAway || snapshot.pulse.boardMoved.length || snapshot.pulse.proximity.length || snapshot.pulse.provisional.length) ? <div className="offline-pulse-list"><p className="kicker">SINCE YOUR LAST SAVED VISIT</p>{snapshot.pulse.sinceAway ? <article><span>{snapshot.pulse.sinceAway.eyebrow}</span><h3>{snapshot.pulse.sinceAway.title}</h3><p>{snapshot.pulse.sinceAway.body}</p></article> : null}{[...snapshot.pulse.boardMoved,...snapshot.pulse.proximity,...snapshot.pulse.provisional].slice(0,8).map((item) => <article key={item.id}><span>{item.eyebrow} · {item.finality}</span><h3>{item.title}</h3><p>{item.body}</p></article>)}</div> : null}
        {snapshot.pulse.whatsHot?.length ? <><h3>What's Hot — saved</h3><div className="universe-now-grid">{snapshot.pulse.whatsHot.slice(0,6).map((event) => <article key={event.eventId}><span>{event.eventType.replaceAll("_"," ")}</span><h3>{event.headline}</h3><p>{event.supportingFact}</p></article>)}</div></> : null}
        {snapshot.pulse.standings?.length ? <><h3>Your saved position</h3><div className="universe-standing-grid">{snapshot.pulse.standings.slice(0,8).map((standing) => <article key={`${standing.categoryId}:${standing.scopeLabel??"all"}`}><span>{standing.categoryTitle}{standing.scopeLabel?` · ${standing.scopeLabel}`:""}</span><strong>#{standing.rank} of {standing.denominator}</strong><p>{standing.label ?? standing.valueLabel}</p></article>)}</div></> : <div className="offline-empty-card">No safe Universe position was saved yet.</div>}
      </> : <div className="offline-empty-card">No Universe snapshot was saved yet. Reconnect for the current field.</div>}
    </section></div> : null}

    {tab === "friends" ? <div className="container player-room-memory"><section className="social-panel">
      <div className="social-panel-heading"><div><p className="kicker">FRIENDS · SAVED</p><h2>Read-only while offline</h2></div>{social?.savedAt ? <span>Saved {savedLabel(social.savedAt)}</span> : null}</div>
      <div className="offline-social-rule"><ShieldCheck size={17}/><p>Reconnect to change Friends. Add, accept, decline, cancel, unfriend and block actions are unavailable offline.</p></div>
      {social?.overview?.friends?.length ? <div className="friend-card-grid">{social.overview.friends.map((friend) => <article className="friend-card" key={friend.playerId}><div className="friend-card-ident"><div className="social-avatar">{friend.canonicalUsername.slice(0,2).toUpperCase()}</div><div><span>FRIEND · SAVED</span><h3>{friend.canonicalUsername}</h3></div></div><div className="friend-card-facts">{friend.latestDeskPeriod?<p><strong>Latest Review</strong>{friend.latestDeskPeriod}</p>:null}{friend.universePlacement?<p><strong>Universe</strong>{friend.universePlacement}</p>:null}{friend.safeHighlight?<p><strong>Recent moment</strong>{friend.safeHighlight}</p>:null}</div></article>)}</div> : <div className="offline-empty-card">No Friends snapshot was saved. Open Friends online once to make recent comparison context available offline.</div>}
      {social?.comparisons?.length ? <div className="offline-comparisons"><p className="kicker">SAVED HEAD-TO-HEAD</p>{social.comparisons.map((comparison) => <article key={comparison.right.playerId}><Swords size={16}/><div><strong>{comparison.left.canonicalUsername} vs {comparison.right.canonicalUsername}</strong><p>{comparison.metrics.slice(0,3).map((metric)=>`${metric.label}: ${metric.leftValue} vs ${metric.rightValue}`).join(" · ") || "No comparable pool sample."}</p></div></article>)}</div> : null}
      <div className="inbox-empty"><Inbox size={20}/><div><strong>Inbox needs a connection.</strong><p>Messages are not synchronized or marked read while offline, and BoardSignal does not save full private message history on this device.</p></div></div>
    </section></div> : null}
  </div>;
}
