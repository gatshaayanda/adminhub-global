...
function assertCanInitiateSocial(account: BoardSignalAccount) {
  if (!canInitiateSocialConnection(account)) {
    throw Object.assign(new Error("Finish setting up your Player Room before changing BoardSignal connections."), { status: 403 });
  }
}

async function rawAccountByPlayerId(playerId: number) {
  const db = getAdminDb();
  ...
  if (account.chessCom?.playerId !== playerId) return undefined;
  return account;
}

async function receivableAccountByPlayerId(playerId: number) {
  const account = await rawAccountByPlayerId(playerId);
  if (!canReceiveSocialConnection(account)) {
    throw Object.assign(new Error("That BoardSignal player is not available for social connection."), { status: 404 });
  }
  return account as BoardSignalAccount;
}

async function interactableAccountByPlayerId(playerId: number) {
  const account = await rawAccountByPlayerId(playerId);
  if (!canInteractSocialConnection(account)) {
    throw Object.assign(new Error("That BoardSignal player is currently unavailable for social interaction."), { status: 404 });
  }
  return account as BoardSignalAccount;
}
...
function relationshipOtherUid(actor: BoardSignalAccount, otherPlayerId: number, relationship: SocialRelationshipRecord) {
  const actorIsA = relationship.playerAId === actor.chessCom.playerId && relationship.playerAUid === actor.uid;
  const actorIsB = relationship.playerBId === actor.chessCom.playerId && relationship.playerBUid === actor.uid;
  if (actorIsA === actorIsB) throw Object.assign(new Error("This social relationship could not be safely resolved."), { status: 409 });
  const storedOtherId = actorIsA ? relationship.playerBId : relationship.playerAId;
  const otherUid = actorIsA ? relationship.playerBUid : relationship.playerAUid;
  if (storedOtherId !== otherPlayerId || !otherUid || otherUid === actor.uid) {
    throw Object.assign(new Error("This social relationship could not be safely resolved."), { status: 409 });
  }
  return otherUid;
}
...
export async function sendFriendRequest(actor: BoardSignalAccount, targetPlayerIdInput: unknown) {
  ...
  assertCanInitiateSocial(actor);
  const target = await receivableAccountByPlayerId(targetPlayerId);
  const db = getAdminDb();
  ...
  if (transition.action === "accept_mutual" && existing) {
    if (!canInteractSocialConnection(target)) throw Object.assign(new Error("This request is still visible, but that player is currently unavailable."), { status: 409 });
...
export async function acceptFriendRequest(actor: BoardSignalAccount, otherPlayerIdInput: unknown) {
  ...
  const other = await interactableAccountByPlayerId(otherPlayerId).catch(() => undefined);
  if (!other) throw Object.assign(new Error("This request is still visible, but that player is currently unavailable."), { status: 409 });
...
export async function declineFriendRequest(actor: BoardSignalAccount, otherPlayerIdInput: unknown) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const otherUid = relationshipOtherUid(actor, otherPlayerId, relationship);
...
export async function blockPlayer(actor: BoardSignalAccount, otherPlayerIdInput: unknown) {
  ...
  const db = getAdminDb();
  const relationshipRef = db.collection("socialRelationships").doc(relationshipId);
  const [relationshipSnapshot, actorProjection] = await Promise.all([
    relationshipRef.get(),
    projection(actor.uid, otherPlayerId),
  ]);

  let otherUid: string;
  let canonicalUsername = "BoardSignal player";
  let avatar: string | undefined;
  if (relationshipSnapshot.exists) {
    const relationship = relationshipSnapshot.data() as SocialRelationshipRecord;
    otherUid = relationshipOtherUid(actor, otherPlayerId, relationship);
    const storedAccount = await rawAccountByPlayerId(otherPlayerId).catch(() => undefined);
    if (storedAccount && storedAccount.uid !== otherUid) {
      throw Object.assign(new Error("This social relationship could not be safely resolved."), { status: 409 });
    }
    if (canExposeStoredSocialIdentity(storedAccount)) {
      canonicalUsername = actorProjection?.canonicalUsername?.trim() || storedAccount!.chessCom.canonicalUsername;
      if (canReceiveSocialConnection(storedAccount)) avatar = actorProjection?.avatar ?? storedAccount!.chessCom.avatar;
    }
  } else {
    assertCanInitiateSocial(actor);
    const other = await receivableAccountByPlayerId(otherPlayerId);
    otherUid = other.uid;
    canonicalUsername = other.chessCom.canonicalUsername;
    avatar = other.chessCom.avatar;
  }
...
export async function setRivalPin(actor: BoardSignalAccount, otherPlayerIdInput: unknown, pinned: boolean) {
  ...
  if (await blocksEitherDirection(actor.chessCom.playerId, otherPlayerId)) throw Object.assign(new Error("This social connection is unavailable."), { status: 403 });
  await interactableAccountByPlayerId(otherPlayerId);
...
async function safePlayerCard(account: BoardSignalAccount, relationship?: SocialProjection, state?: Awaited<ReturnType<typeof loadActiveUniverseState>>): Promise<SocialPlayerCard> {
  ...
  const publicUniverseAllowed = account.privacy?.universeCoverage !== false && account.privacy?.publicPlayerPage !== false;
  const publicMoments = publicUniverseAllowed
    ? await getAdminDb().collection("publicShareMoments").where("playerId", "==", String(account.chessCom.playerId)).limit(8).get().catch(() => ({ docs: [] }))
    : { docs: [] };
  ...
  const participant = publicUniverseAllowed && latest?.desk ? deskToUniverseParticipant(latest.desk) : undefined;
...
export async function searchSocialPlayers(actor: BoardSignalAccount, queryInput: unknown) {
  ...
  const accounts = users.docs.map((document) => document.data() as BoardSignalAccount)
    .filter(canReceiveSocialConnection)
...
async function unavailableCardForProjection(actor: BoardSignalAccount, item: SocialProjection) {
  const id = canonicalSocialRelationshipId(actor.chessCom.playerId, item.otherPlayerId);
  const relationshipSnapshot = await getAdminDb().collection("socialRelationships").doc(id).get();
  if (!relationshipSnapshot.exists) return undefined;
  const relationship = relationshipSnapshot.data() as SocialRelationshipRecord;
  if (!relationshipProjectionMatches(actor.chessCom.playerId, actor.uid, item, relationship)) return undefined;
  const storedAccount = await rawAccountByPlayerId(item.otherPlayerId).catch(() => undefined);
  return unavailableRelationshipCard(item, canExposeStoredSocialIdentity(storedAccount));
}
...
export async function socialOverview(actor: BoardSignalAccount) {
  ...
  const cards = await Promise.all(visible.map(async (item) => {
    const account = await interactableAccountByPlayerId(item.otherPlayerId).catch(() => undefined);
    if (account) return safePlayerCard(account, item, state);
    return unavailableCardForProjection(actor, item);
  }));
  ...
  const friendIds = new Set(friends.filter((item) => item.availability !== "unavailable").map((item) => String(item.playerId)));
  ...
  for (const friend of friends) {
    if (friend.availability === "unavailable") continue;
    const friendAccount = await interactableAccountByPlayerId(friend.playerId).catch(() => undefined);
...
export async function headToHead(actor: BoardSignalAccount, otherPlayerIdInput: unknown) {
  ...
  const other = await interactableAccountByPlayerId(otherPlayerId);
