...
export default function PlayerFriends({ token, initialComparePlayerId, onChanged }: Props) {
  ...
  return <section className="friends-surface">
    ...
    {overview.incoming.length || overview.outgoing.length ? <section className="social-panel"><p className="kicker">REQUESTS</p><div className="friend-card-grid">
      {overview.incoming.map((player) => <FriendCard key={`in:${player.playerId}`} player={player} status="incoming" actions={<><button className="button button-lime" type="button" disabled={Boolean(busy)} onClick={() => socialAction("accept", player.playerId)}><Check size={15}/> Accept</button><button className="button button-quiet" type="button" disabled={Boolean(busy)} onClick={() => socialAction("decline", player.playerId)}><X size={15}/> Decline</button></>} />)}
      {overview.outgoing.map((player) => <FriendCard key={`out:${player.playerId}`} player={player} status="outgoing" actions={<button className="button button-quiet" type="button" disabled={Boolean(busy)} onClick={() => socialAction("cancel", player.playerId)}><X size={15}/> Cancel</button>} />)}
    </div></section> : null}
    ...
    {overview.friends.length ? <section className="social-panel"><div className="social-panel-heading"><p className="kicker">FRIENDS</p><span>{overview.friends.length} connected</span></div><div className="friend-card-grid">{overview.friends.map((player) => <FriendCard key={player.playerId} player={player} status="friends" actions={<>
      ...
      <button className="button button-quiet" type="button" disabled={Boolean(busy)} onClick={() => socialAction("pin", player.playerId, !player.rivalPinned)}>{player.rivalPinned ? "Unpin rival" : "Pin to Rival Watch"}</button>
      <button className="button button-quiet" type="button" disabled={Boolean(busy)} onClick={() => socialAction("unfriend", player.playerId)}><UserMinus size={15}/> Unfriend</button>
      <button className="button button-quiet" type="button" disabled={Boolean(busy)} onClick={() => socialAction("block", player.playerId)}><Ban size={15}/> Block</button>
    </>} />)}</div></section> : null}

    {overview.rivalWatch.length ? <section className="social-panel rival-watch-panel"><p className="kicker">RIVAL WATCH</p><div className="rival-watch-grid">{overview.rivalWatch.map((item) => <article key={item.player.playerId}><span>{item.label}</span><h3>{item.player.canonicalUsername}</h3><p>{item.detail}</p><button type="button" className="text-link social-text-button" onClick={() => compare(item.player.playerId)}>Open Head-to-Head</button></article>)}</div></section> : null}
...
function FriendCard({ player, status, actions }: { player: SocialPlayerCard; status: "incoming" | "outgoing" | "friends"; actions: ReactNode }) {
  return <article className="friend-card"><div className="friend-card-ident"><div className="social-avatar">{player.avatar ? <img src={player.avatar} alt=""/> : <CircleUserRound aria-hidden="true"/>}</div><div><span>{status === "friends" ? "FRIEND" : status === "incoming" ? "REQUEST RECEIVED" : "REQUEST SENT"}</span><h3>{player.canonicalUsername}</h3></div></div><div className="friend-card-facts">{player.latestDeskPeriod ? <p><strong>Latest Desk</strong>{player.latestDeskPeriod}</p> : null}{player.primaryPool ? <p><strong>Primary pool</strong>{player.primaryPool}</p> : null}{player.universePlacement ? <p><strong>Universe</strong>{player.universePlacement}</p> : null}{player.safeHighlight ? <p><strong>Recent moment</strong>{player.safeHighlight}</p> : null}</div><div className="friend-card-actions">{actions}<Link href={`/player/${encodeURIComponent(player.canonicalUsername)}`} className="text-link">Open player <ArrowRight size={14}/></Link></div></article>;
}

function DiscoveryCard({ player, status, busy, onAction, onCompare }: { player: SocialPlayerCard; status?: string; busy: boolean; onAction: (action: string, playerId: number, pinned?: boolean) => Promise<void>; onCompare: (playerId: number) => Promise<void> }) {
  return <article className="friend-card discovery-card"><div className="friend-card-ident"><div className="social-avatar">{player.avatar ? <img src={player.avatar} alt=""/> : <CircleUserRound aria-hidden="true"/>}</div><div><span>BOARDSIGNAL PLAYER</span><h3>{player.canonicalUsername}</h3></div></div>{player.safeHighlight ? <p>{player.safeHighlight}</p> : <p>{player.universePlacement ?? "Active Founding Beta player."}</p>}<div className="friend-card-actions">{status === "friends" ? <button className="button button-blue" type="button" disabled={busy} onClick={() => onCompare(player.playerId)}><Swords size={15}/> Compare</button> : status === "outgoing" ? <button className="button button-quiet" type="button" disabled>Request pending</button> : status === "incoming" ? <button className="button button-lime" type="button" disabled={busy} onClick={() => onAction("accept", player.playerId)}><Check size={15}/> Accept request</button> : <button className="button button-lime" type="button" disabled={busy} onClick={() => onAction("send", player.playerId)}><UserPlus size={15}/> Add Friend</button>}<Link href={`/player/${encodeURIComponent(player.canonicalUsername)}`} className="text-link">Open player</Link></div></article>;
}

function HeadToHead({ comparison, onClose }: { comparison: HeadToHeadPayload; onClose: () => void }) {
  return <section className="head-to-head" aria-labelledby="head-to-head-title"><div className="head-to-head-heading"><div><p className="kicker">HEAD TO HEAD</p><h2 id="head-to-head-title">{comparison.left.canonicalUsername} <span>vs</span> {comparison.right.canonicalUsername}</h2><p>{comparison.recentFourLabel}. Pools stay separate. Old Desks leave the comparison when they leave active four-Desk memory.</p></div><button type="button" className="button button-quiet" onClick={onClose}>Close</button></div><div className="head-to-head-ident"><article><span>{comparison.left.desksAvailable} active Desks</span><strong>{comparison.left.canonicalUsername}</strong></article><div aria-hidden="true">VS</div><article><span>{comparison.right.desksAvailable} active Desks</span><strong>{comparison.right.canonicalUsername}</strong></article></div>{comparison.comparablePools.length ? <p className="comparison-pools">Comparable pools: {comparison.comparablePools.map((pool) => pool.toUpperCase()).join(" · ")}</p> : <div className="social-empty-note"><Shield size={16}/><p>No comparable pool sample is currently available. BoardSignal will not manufacture a winner.</p></div>}<div className="comparison-grid">{comparison.metrics.map((metric) => <article key={metric.key}><div><span>{metric.label}</span>{metric.scope ? <b>{metric.scope}</b> : null}</div><div className="comparison-values"><strong>{metric.leftValue}</strong><span>vs</span><strong>{metric.rightValue}</strong></div>{metric.note ? <p>{metric.note}</p> : null}</article>)}</div>{comparison.universe.length ? <div className="comparison-universe"><p className="kicker">UNIVERSE</p>{comparison.universe.slice(0,6).map((item) => <article key={`${item.categoryId}:${item.scopeLabel ?? "all"}`}><span>{item.categoryTitle}{item.scopeLabel ? ` · ${item.scopeLabel}` : ""}</span><strong>#{item.leftRank} vs #{item.rightRank}</strong><p>{item.note ?? `${item.leftValueLabel} · ${item.rightValueLabel}`}</p></article>)}</div> : null}<div className="social-private-rule"><Shield size={17}/><p><strong>Public coverage. Private weakness.</strong> Head-to-Head excludes Red, private Amber, Blue, evidence, recurrence, contact information and private messages. Game links remain controlled by each player's existing public-game-link preference.</p></div></section>;
}
