...
async function playerSnapshot(account: OperationsAccount) {
  ...
  // Historical imports are genuine private Reviews, but this console's
  // operational review/retention counts intentionally retain their pre-H
  // meaning: Original Beta + organic LIVE lifecycle only.
  ...
  return {
    ...
    latestReview: latestVerified ? {
      ...
      // Historical imports never become REVIEW READY merely because written now.
      publishedAt: latestVerified.period.source === "historical" ? undefined : latestVerified.data.publishedAt,
    } : undefined,
...
export async function founderOperationsSnapshot(now = new Date()) {
  ...
  const historicalActivationPlayers = activeAccounts.filter((account) => {
    const hasBackfillActivation = Boolean(account.reviewHistoryBackfill?.establishedAt);
    const hadOriginal = account.originalBetaPlayer || Object.keys(account.originalBetaHistoryPeriods ?? {}).length > 0;
