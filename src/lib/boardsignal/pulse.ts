...
export type PulseUniverseBoard = UniverseBoard & { fieldLabel: UniverseFieldLabel; comparableLivePlayers: number };
export type PulseUniverseGroup = Omit<UniverseCategoryGroup, "boards"> & { boards: PulseUniverseBoard[] };
...
export function latestUniverseParticipants(participants: UniverseParticipant[]) {
  const latest = new Map<string, UniverseParticipant>();
  for (const participant of participants) {
    const key = participant.stablePlayerId ? `player:${participant.stablePlayerId}` : `name:${normalizeUsername(participant.player)}`;
    const existing = latest.get(key);
    if (!existing
      || participant.periodEnd > existing.periodEnd
      || (participant.periodEnd === existing.periodEnd && participant.id.localeCompare(existing.id) < 0)) {
      latest.set(key, participant);
    }
  }
...
function boardMap(participants: UniverseParticipant[]) {
  // Period chronology is authoritative. Collapse to one latest participant
  // before scoring/ranking so an older stronger historical week cannot win the
  // dedupe and represent the same real player on a current board.
  return new Map(buildUniverseBoards(latestUniverseParticipants(participants)).map((board) => [board.key, dedupeBoardEntries(board)]));
}
...
export function buildActiveUniverseBoards(
  ...
  return [...keys].sort().flatMap((key) => {
    ...
    return [{ ...selected, fieldLabel: transitioned ? "BOARDSIGNAL FIELD" : "FOUNDING BETA FIELD", comparableLivePlayers } satisfies PulseUniverseBoard];
  });
}
...
export function standingsFromActiveBoards(boards: PulseUniverseBoard[], participantId: string): DeskUniverseStanding[] {
  return boards.flatMap((board) => {
    ...
    return [{
      ...
      nearestAbove: index > 0 ? { player: board.entries[index - 1].player, valueLabel: board.entries[index - 1].valueLabel } : undefined,
...
export function standingSnapshots(boards: PulseUniverseBoard[], participantId: string): PulseStandingSnapshot[] {
  return boards.flatMap((board) => {
    ...
    if (!item) return [];
    return [{ key: board.key, categoryId: board.categoryId, categoryTitle: board.title, scopeLabel: board.scopeLabel, rank: item.rank, denominator: board.entries.length, value: item.value, valueLabel: item.valueLabel } satisfies PulseStandingSnapshot];
  });
}

export function deriveCurrentEpisodeDelta(previous: CurrentEpisodeSummary | undefined, current: CurrentEpisodeSummary | undefined): PlayerPulseCard | undefined {
  ...
  for (const pool of current.pools) {
    ...
    const ratingMovement = pool.ratingEnd !== undefined && before.ratingEnd !== undefined
      ...
      : pool.ratingDelta !== undefined && before.ratingDelta !== undefined ? pool.ratingDelta - before.ratingDelta : undefined;
...
export function deriveBoardMovement(previous: PulseStandingSnapshot[], current: PulseStandingSnapshot[]): PlayerPulseCard[] {
...
export function deriveProximityCards(boards: PulseUniverseBoard[], participantId: string): PlayerPulseCard[] {
  ...
  for (const board of boards) {
    ...
    if (above) {
      ...
      if (gap) cards.push({ id: `in-reach:${board.key}:${above.participantId}`, kind: "in-reach", eyebrow: "IN REACH", title: `${board.title}${board.scopeLabel ? ` · ${board.scopeLabel}` : ""}`, body: `${gap} Current official positions: you #${current.rank}, ${above.player} #${above.rank}.`, categoryId: board.categoryId, pool: board.scopeLabel, finality: "official" });
    }
    const below = board.entries[index + 1];
    if (below) {
      const gap = reachableGap(board, below, current);
      if (gap) cards.push({ id: `on-radar:${board.key}:${below.participantId}`, kind: "on-radar", eyebrow: "ON YOUR RADAR", title: `${below.player} is close in ${board.title}${board.scopeLabel ? ` · ${board.scopeLabel}` : ""}.`, body: `You are #${current.rank}; ${below.player} is #${below.rank}. ${gap}`, categoryId: board.categoryId, pool: board.scopeLabel, finality: "official" });
    }
  }
...
export function currentEpisodeToProvisionalParticipant(username: string, playerId: string, current: CurrentEpisodeSummary): UniverseParticipant {
  return {
    ...
    pools: current.pools.flatMap((pool) => {
      ...
      return [{ pool: normalized, games: pool.games, start: pool.ratingStart, end: pool.ratingEnd, change: pool.ratingDelta }];
    }),
  };
}

export function deriveProvisionalCards(boards: PulseUniverseBoard[], provisionalParticipantId: string): PlayerPulseCard[] {
  return boards.flatMap((board) => {
...
function eventAgeMs(event: PublicUniverseEvent, now: Date) { return Math.max(0, now.getTime() - Date.parse(event.publishedAt || event.occurredAt)); }
...
export function universeHotnessScore(event: PublicUniverseEvent, now = new Date()) {
  ...
  const rankImpact = event.rankAfter ? event.rankAfter === 1 ? 45 : event.rankAfter <= 3 ? 32 : event.rankBefore && event.rankAfter < event.rankBefore ? 20 : 8 : 0;
  const novelty: Record<UniverseEventType, number> = {
    new_player: 20, first_desk: 34, desk_completed: 8, new_leader: 50, entered_top3: 42, podium_move: 34, rank_move: 20,
    rating_climb: 24, rating_recovery: 24, winning_run: 26, strong_finish: 24, breakthrough: 30, best_upset: 28,
    moment_of_the_week: 30, player_to_watch: 24,
  };
...
export function rankWhatsHot(events: PublicUniverseEvent[], now = new Date(), limit = 8) {
  const eligible = events
    ...
    .sort((a, b) => b.score - a.score || b.event.publishedAt.localeCompare(a.event.publishedAt) || a.event.eventId.localeCompare(b.event.eventId));
...
export function nominateShareMoments(desk: BoardSignalDesk, standings: DeskUniverseStanding[] = [], now = new Date()): SafeShareMoment[] {
  ...
  const base = {
    playerId: String(desk.player.playerId), canonicalUsername: desk.player.username, avatar: desk.player.avatar,
    deskKey: `${desk.player.playerId}:${desk.period.start}:${desk.period.end}`, periodStart: desk.period.start, periodEnd: desk.period.end,
    periodLabel: desk.period.label, createdAt: now.toISOString(), dataMode: "live" as const, safePublic: true as const,
  };
  for (const pool of desk.pools) {
    const normalized = normalizePool(pool.pool);
    const delta = pool.change ?? (pool.firstRecordedRating !== undefined && pool.lastRecordedRating !== undefined ? pool.lastRecordedRating - pool.firstRecordedRating : undefined);
    if (!normalized || pool.games < 3 || delta === undefined || delta <= 0) continue;
    candidates.push({ ...base, id: shareMomentId(desk, `rating_${normalized}`), pool: normalized, headline: `${delta > 0 ? "+" : ""}${delta} ${normalized[0].toUpperCase() + normalized.slice(1)}`, supportingFact: `${pool.games} ${normalized} games in the completed seven-day Review produced a recorded ${delta > 0 ? "+" : ""}${delta} rating movement.`, statLabel: "Rating movement", statValue: `+${delta}`, categoryId: "rating-climb", score: 80 + Math.min(delta, 100) });
  }
  if (desk.longestWinStreak >= 3) candidates.push({ ...base, id: shareMomentId(desk, "winning_run"), headline: `${desk.longestWinStreak} straight wins`, supportingFact: `${desk.longestWinStreak} consecutive wins were recorded inside the completed seven-day Review.`, statLabel: "Winning run", statValue: `${desk.longestWinStreak} straight`, categoryId: "winning-run", score: 120 + desk.longestWinStreak });
  ...
  if (finalDay) {
    ...
    if (games >= 3 && score > 50) candidates.push({ ...base, id: shareMomentId(desk, "strong_finish"), headline: `${finalDay.wins} wins from the final ${games} games`, supportingFact: `The final active day closed ${finalDay.wins}W · ${finalDay.draws}D · ${finalDay.losses}L.`, statLabel: "Strong finish", statValue: `${score.toFixed(0)}% score`, categoryId: "strong-finish", score: 95 + score });
  }
  if ((desk.checkmateWins ?? 0) >= 3) candidates.push({ ...base, id: shareMomentId(desk, "checkmate_finish"), headline: `${desk.checkmateWins} checkmate wins`, supportingFact: `${desk.checkmateWins} wins in the completed Review ended by checkmate.`, statLabel: "Checkmate finishes", statValue: String(desk.checkmateWins), categoryId: "checkmate-finish", score: 70 + Number(desk.checkmateWins) });
  for (const standing of standings.filter((item) => item.rank <= 3)) candidates.push({ ...base, id: shareMomentId(desk, `top3_${standing.categoryId}_${standing.scopeLabel ?? "all"}`), headline: `#${standing.rank} ${standing.categoryTitle}${standing.scopeLabel ? ` · ${standing.scopeLabel}` : ""}`, supportingFact: `${standing.valueLabel} placed this completed Review #${standing.rank} of ${standing.denominator} in the current official comparison field.`, statLabel: standing.categoryTitle, statValue: `#${standing.rank}`, categoryId: standing.categoryId as UniverseCategoryId, score: 150 - standing.rank });
  const seen = new Set<string>();
  return candidates.sort((a, b) => b.score - a.score || a.id.localeCompare(b.id)).filter((item) => {
    const key = `${item.categoryId}:${item.pool ?? "all"}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  }).slice(0, 3).map(({ score: _score, ...moment }) => moment);
}

export function shareMomentUrl(origin: string, momentId: string) { return `${origin.replace(/\/$/, "")}/share/${encodeURIComponent(momentId)}`; }
...
export function deskParticipantId(desk: BoardSignalDesk) { return deskToUniverseParticipant(desk)?.id; }
