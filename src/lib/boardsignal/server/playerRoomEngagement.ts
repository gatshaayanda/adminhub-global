...
import type {
  BoardSignalAccount,
  BoardSignalPlayerRoomEngagement,
  BoardSignalTrustpilotResolution,
...
function cleanSessionId(value: unknown) {
  ...
  if (!/^[A-Za-z0-9:_-]{8,180}$/.test(sessionId)) {
    throw new PlayerRoomEngagementError("PLAYER_ROOM_SESSION_INVALID", "That Player Room session is invalid.");
  }
  return sessionId;
}
...
function cleanForegroundSeconds(value: unknown) {
  ...
  if (!Number.isFinite(seconds) || seconds < 0) {
    throw new PlayerRoomEngagementError("PLAYER_ROOM_SUMMARY_INVALID", "That Player Room summary is invalid.");
  }
...
function promptFor(
  ...
  founder: boolean,
): PlayerRoomTrustpilotPrompt | null {
...
export async function recordPlayerRoomEntry(uid: string, input: unknown) {
  ...
  const outcome = await db.runTransaction(async (transaction) => {
    ...
    if (account.accessStatus !== "active") {
      throw new PlayerRoomEngagementError("PLAYER_ROOM_ACCESS_INACTIVE", "This BoardSignal access is not active.", 403);
    }
    ...
    if (prompt === "first" && !invitation?.firstAskShownAt) {
      nextInvitation = { ...invitation, firstAskShownAt: nowIso };
    }
    if (prompt === "final" && !invitation?.finalAskShownAt) {
      nextInvitation = { ...invitation, finalAskShownAt: nowIso };
    }
...
export async function resolveTrustpilotFollowUp(uid: string, input: unknown) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (isFounder(account)) {
      throw new PlayerRoomEngagementError("TRUSTPILOT_FOUNDER_EXCLUDED", "Founder activity is excluded from automated Trustpilot asks.", 403);
    }
    if (currentVisitCount(account.playerRoomEngagement?.roomVisitCount) < 6) {
      throw new PlayerRoomEngagementError("TRUSTPILOT_FOLLOW_UP_TOO_EARLY", "The final Trustpilot follow-up is not available yet.", 409);
    }

    const invitation = account.trustpilotReviewInvitation;
    if (trustpilotCycleComplete(invitation)) {
      return {
        status: "already_resolved" as const,
        resolution: invitation?.resolution,
        invitationConfirmed: Boolean(invitation?.confirmedAt),
      };
    }

    const nowIso = new Date().toISOString();
    const next: BoardSignalTrustpilotReviewInvitation = {
      ...invitation,
      resolution,
      resolvedAt: nowIso,
      automaticCycleCompletedAt: nowIso,
    };
    transaction.set(ref, { trustpilotReviewInvitation: next }, { merge: true });
    return {
      status: "resolved" as const,
      resolution,
      invitationConfirmed: Boolean(next.confirmedAt),
    };
  });
}
