...
test('2. Current BoardSignal remains the live experience and Guidance points to upcoming games', () => {
  ...
  assert.match(currentEngagement, /Current BoardSignal live context/);
});
...
test('6. M3 visit deduplication remains intact', () => {
  assert.match(playerRoomEngagement, /lastCountedSessionId === sessionId/);
  assert.match(playerRoomEngagement, /previousVisitCount: count/);
  assert.match(playerRoomEngagement, /nextVisitCount: nextCount/);
});
...
test('7. M4 explicit-action engagement behavior remains intact', () => {
  ...
  assert.match(guideRoute, /body\.mode !== "beta_preview"/);
});
