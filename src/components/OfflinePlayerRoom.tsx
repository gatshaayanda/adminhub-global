...
export default function OfflinePlayerRoom({ uid: suppliedUid, initialSnapshot, embedded = false }: { uid?: string; initialSnapshot?: OfflinePlayerRoomSnapshot; embedded?: boolean }) {
  ...
  return <div id="main" className={`offline-player-room player-room-authenticated ${embedded ? "is-embedded" : ""}`}>
    ...
    <nav className="container offline-room-tabs" aria-label="Saved My BoardSignal">{(["desk","progress","universe","friends"] as OfflineTab[]).map((item) => <button type="button" key={item} className={tab===item?"active":""} onClick={() => setTab(item)} aria-current={tab===item?"page":undefined}>{item === "desk" ? "Review" : item === "progress" ? "Progress" : item === "universe" ? "Universe" : "Friends"}</button>)}</nav>
    ...
    {tab === "universe" ? <div className="container player-room-memory"><section className="pulse-universe-block"><div className="pulse-block-heading"><div><p className="kicker">UNIVERSE · SAVED</p><h2>Last synchronized {savedLabel(snapshot.lastSyncedAt)}</h2></div><span>Not live</span></div>{snapshot.pulse ? <><h3>Since your last synchronization</h3><div className="offline-pulse-list">{snapshot.pulse.sinceAway ? <article><span>{snapshot.pulse.sinceAway.eyebrow}</span><h3>{snapshot.pulse.sinceAway.title}</h3><p>{snapshot.pulse.sinceAway.body}</p></article> : null}{[...snapshot.pulse.boardMoved,...snapshot.pulse.proximity,...snapshot.pulse.provisional].slice(0,8).map((item) => <article key={item.id}><span>{item.eyebrow} · {item.finality === "provisional" ? "forming" : "official"}</span><h3>{item.title}</h3><p>{item.body}</p></article>)}</div></> : null}{snapshot.pulse?.whatsHot?.length ? <><h3>What's Hot — saved</h3><div className="universe-now-grid">{snapshot.pulse.whatsHot.slice(0,6).map((event) => <article key={event.eventId}><span>{event.eventType.replaceAll("_"," ")}</span><h3>{event.headline}</h3><p>{event.supportingFact}</p></article>)}</div></> : null}{snapshot.pulse?.standings?.length ? <><h3>Saved standings</h3><div className="universe-standing-grid">{snapshot.pulse.standings.slice(0,8).map((standing) => <article key={`${standing.categoryId}:${standing.scopeLabel??"all"}`}><span>{standing.categoryTitle}{standing.scopeLabel?` · ${standing.scopeLabel}`:""}</span><strong>#{standing.rank} of {standing.denominator}</strong><p>{standing.label ?? standing.valueLabel}</p></article>)}</div></> : <div className="offline-empty-card">No safe Universe snapshot was saved yet.</div>}</section></div> : null}
