...
export default function FounderCommunications() {
  ...
  }), [allowReply, audienceKind, body, browserPush, email, link, manualContact, segment, selected, title, type]);
  ...
  return <>
    <section className="desk-section founder-composer">
      <div className="room-section-heading"><div><p className="kicker">COMMUNICATIONS</p><h2>Send a BoardSignal message</h2><p>In-app delivery is always available. Browser push is optional and permission-gated. Email is optional and only becomes selectable when the server provider is configured and the player is eligible.</p></div><button className="button button-outline" type="button" onClick={loadFriendsRivalsReleaseDraft}>Load Friends & Rivals release draft</button></div>
