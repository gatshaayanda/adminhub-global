...
import {
  ...
  shouldReplaceIncumbentSignal,
  stateForNewCandidate,
} from "@/lib/boardsignal/coaching";
import { getAdminDb } from "@/utils/firebaseAdmin";

export function validStoredCoachingState(value: unknown): BoardSignalCoachingState | undefined {
  if (!value || typeof value !== "object") return undefined;
  const state = value as Partial<BoardSignalCoachingState>;
  ...
  return state as BoardSignalCoachingState;
}

export async function reconcilePlayerCoachingState(
  uid: string,
  episode: CurrentEpisodeWithNextGameGuidance,
): Promise<BoardSignalCoachingPresentation | undefined> {
  const candidate = coachingCandidateForEpisode(episode);
  const db = getAdminDb();
  const ref = db.collection("users").doc(uid);
  const nowIso = new Date().toISOString();
  const state = await db.runTransaction(async (transaction: any) => {
    const snapshot = await transaction.get(ref);
    if (!snapshot.exists) return undefined;
    const account = snapshot.data() as BoardSignalAccount;
    const existing = validStoredCoachingState(account.coachingState);
    if (!candidate) {
      if (!existing) return undefined;
      if (existing.periodStart !== episode.periodStart || existing.family === "loss_run") {
        transaction.set(ref, { coachingState: FieldValue.delete() }, { merge: true });
        return undefined;
      }
      ...
      transaction.set(ref, { coachingState: retained }, { merge: true });
      return retained;
    }
    let next: BoardSignalCoachingState;
  ...
  return state ? presentationFromCoachingState(state) : undefined;
}

export function episodeWithCanonicalCoaching(
  episode: CurrentEpisodeWithNextGameGuidance,
  coaching: BoardSignalCoachingPresentation | undefined,
  ...
  const guidance = {
    ...
    supportingFacts,
    reinforcement: coaching.source === "current_week_reinforces_previous_review" && episode.nextGameGuidance.source === "current_week_reinforces_previous_review"
      ? episode.nextGameGuidance.reinforcement
      : undefined,
    reason: undefined,
    previousReviewPeriod: coaching.previousReviewPeriod ?? episode.nextGameGuidance.previousReviewPeriod,
    latestGameNote: episode.latestGame
      ? latestMatch
        ? "Your latest game also contains evidence for the coaching signal BoardSignal is keeping in front of you."
        : "BoardSignal saw your latest game. The incumbent coaching signal remains the main supported cue for now."
      : episode.nextGameGuidance.latestGameNote,
    m7: {
      coachingSignalKey: coaching.coachingSignalKey,
      level: coaching.level,
      provenance: coaching.provenance,
      cueTitle: coaching.cueTitle,
      cueCopy: coaching.cueCopy,
      level2Copy: coaching.level2Copy,
      evidenceCount: coaching.evidenceCount,
      gamesConsidered: coaching.gamesConsidered,
  ...
  const latestGame = episode.latestGame
    ? {
        ...episode.latestGame,
        supportsSelectedGuidance: Boolean(latestMatch),
        supportingSummary: latestMatch?.summary,
      }
    : undefined;
  return { ...episode, nextGameGuidance: guidance, latestGame };
}
