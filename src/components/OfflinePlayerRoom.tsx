...
export default function OfflinePlayerRoom({ uid: suppliedUid, initialSnapshot, embedded = false }: { uid?: string; initialSnapshot?: OfflinePlayerRoomSnapshot; embedded?: boolean }) {
  ...
  return <div id="main" className={`offline-player-room ${embedded ? "is-embedded" : ""}`}>
    ...
    {tab === "desk" ? <>
      ...
      {selectedDesk ? <div aria-label="Saved Review read only"><UniversalPlayerDesk requestedUsername={selectedDesk.desk.player.username} publishedDesk={selectedDesk.desk} publishedEngineResults={selectedDesk.engineResults}/></div> : <div className="container offline-empty-card">No completed Review was saved yet.</div>}
    </> : null}
