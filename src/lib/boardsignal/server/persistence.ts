...
export async function publishPrivateDesk(token: DecodedIdToken, desk: BoardSignalDesk, engineResults: Record<string, DeskEngineResult>, options: PublishOptions = {}) {
  ...
  if (existingDocument) {
    // Deterministic period identity wins every race. Never overwrite a Review
    // that another tab/request has already persisted for this player-period.
  ...
  if (historical) {
    if (initialHistorical) {
      const evaluatedAt = storedAt;
      accountUpdate.cadenceAnchor = desk.period.start;
      accountUpdate.nextDeskDueAt = desk.cadence?.nextAvailableOn;
      accountUpdate.reviewHistoryBackfill = {
        version: REVIEW_HISTORY_BACKFILL_VERSION,
        status: "pending",
        targetPeriods: fourPeriodWindow(desk.period.start),
        evaluated: { [desk.period.start]: { status: "published", evaluatedAt } },
