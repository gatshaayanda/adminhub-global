import "server-only";
import type {
  BoardSignalCoachingFeedbackItem,
  BoardSignalCoachingLevel,
  BoardSignalCurrentFeedback,
  BoardSignalCurrentReaction,
  BoardSignalLegacyCurrentFeedbackItem,
...
function cleanLegacyItemKey(value: unknown) {
  const text = typeof value === "string" ? value.trim() : "";
...
function cleanSignalKey(value: unknown) {
  const text = typeof value === "string" ? value.trim() : "";
  if (!/^m7:\d{4}-\d{2}-\d{2}:(?:current|previous):[A-Za-z0-9:_-]{1,150}$/.test(text) || text.length > 220) {
    throw new CurrentBoardSignalFeedbackError("CURRENT_FEEDBACK_INVALID_SIGNAL", "That coaching signal is invalid.");
  }
  return text;
}
...
function cleanReaction(value: unknown): BoardSignalCurrentReaction {
  ...
  throw new CurrentBoardSignalFeedbackError("CURRENT_FEEDBACK_INVALID_REACTION", "Choose whether this coaching explanation was helpful.");
}

function isLegacyItem(value: unknown): value is BoardSignalLegacyCurrentFeedbackItem {
...
function isM7Item(value: unknown): value is BoardSignalCoachingFeedbackItem {
  if (!value || typeof value !== "object") return false;
...
function feedbackForPeriod(existing: BoardSignalCurrentFeedback | undefined, periodStart: string, periodEnd: string) {
  return existing?.periodStart === periodStart && existing.periodEnd === periodEnd
    ? normalizeCurrentFeedbackItems(existing.items, MAX_CURRENT_FEEDBACK_ITEMS)
    : [];
}

async function recordLegacyFeedback(uid: string, body: Record<string, unknown>) {
  ...
  const itemKey = cleanLegacyItemKey(body.itemKey);
  ...
  const outcome = await db.runTransaction(async (transaction: any) => {
    ...
    const existingItems = feedbackForPeriod(account.currentBoardSignalFeedback, periodStart, periodEnd);
    const existing = existingItems.find((item): item is BoardSignalLegacyCurrentFeedbackItem => isLegacyItem(item) && item.itemKey === itemKey);
    if (existing?.reaction === reaction) return { feedback: { periodStart, periodEnd, items: existingItems }, changed: false as const };
    const nowIso = new Date().toISOString();
    const item: BoardSignalLegacyCurrentFeedbackItem = { itemKey, reaction, reactedAt: nowIso };
    const items = [item, ...existingItems.filter((candidate) => !(isLegacyItem(candidate) && candidate.itemKey === itemKey))].slice(0, MAX_CURRENT_FEEDBACK_ITEMS);
    const feedback = { periodStart, periodEnd, items } satisfies BoardSignalCurrentFeedback;
    transaction.set(ref, { currentBoardSignalFeedback: feedback }, { merge: true });
    return { feedback, changed: true as const, previousReaction: existing?.reaction, at: nowIso };
  });
  if (outcome.changed) await recordFounderFeedbackProjection({ uid, reaction, previousReaction: outcome.previousReaction, at: outcome.at }).catch(() => undefined);
  return { feedback: outcome.feedback, coaching: undefined };
}
...
export async function recordCurrentBoardSignalFeedback(uid: string, input: unknown) {
  ...
  const outcome = await db.runTransaction(async (transaction: any) => {
    const snapshot = await transaction.get(ref);
    if (!snapshot.exists) throw new CurrentBoardSignalFeedbackError("CURRENT_FEEDBACK_ACCOUNT_NOT_FOUND", "This BoardSignal account could not be found.", 404);
    const account = snapshot.data() as { accessStatus?: string; currentBoardSignalFeedback?: BoardSignalCurrentFeedback; coachingState?: unknown };
    if (account.accessStatus && account.accessStatus !== "active") throw new CurrentBoardSignalFeedbackError("CURRENT_FEEDBACK_ACCESS_INACTIVE", "This BoardSignal access is not active.", 403);
    ...
    const items = [item, ...existingItems.filter((candidate) => !(isM7Item(candidate) && candidate.coachingSignalKey === coachingSignalKey && candidate.level === level))].slice(0, MAX_CURRENT_FEEDBACK_ITEMS);
    const feedback = { periodStart: state.periodStart, periodEnd: state.periodEnd, items } satisfies BoardSignalCurrentFeedback;
