...
function factualEpisodeCheckpoint(episode: Awaited<ReturnType<typeof buildCurrentEpisodeSummary>>): CurrentEpisodeSummary { const { nextGameGuidance, latestGame, ...factualEpisode } = episode; void nextGameGuidance; void latestGame; return factualEpisode; }
...
export async function GET(request: Request) {
  try {
    ...
    catch (error) { progressUnavailable = error instanceof Error ? error.message : "Current episode progress is temporarily unavailable."; }
    ...
    await recordGuidePlayerRoomSnapshot(account, { currentEpisode, latestDesk: snapshot.desks[0]?.desk, recentDeskLabels: snapshot.reviewHistory.map((item) => item.periodLabel), pulse: snapshot.pulse, shareMoments: snapshot.shareMoments }).catch(() => undefined);
...
export async function POST(request: Request) {
  try {
    const token = await requirePlayerToken(request);
    const body = await request.json() as { action?: "acceptAgreement" | "saveFactualReview" | "publishDesk" | "updatePreferences"; desk?: BoardSignalDesk; engineResults?: Record<string, DeskEngineResult>; reviewLifecycle?: ReviewLifecycle; historyLeaseId?: string; privacy?: import("@/lib/boardsignal/account").BoardSignalPrivacySettings; notificationPreferences?: import("@/lib/boardsignal/account").BoardSignalNotificationPreferences; contact?: { preferredContactMethod: import("@/lib/boardsignal/account").BoardSignalContactMethod; preferredContactValue: string; betaContactConsent: boolean } };
    if (body.action === "acceptAgreement") return response({ ok: true, account: await acceptFoundingBetaAgreement(token) });
    if (body.action === "saveFactualReview" && body.desk) return response({ ok: true, factualReview: await savePendingFactualReview(token, body.desk, { reviewLifecycle: body.reviewLifecycle, historyLeaseId: body.historyLeaseId }) });
