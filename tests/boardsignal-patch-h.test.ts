...
test("6-10 existing player: cadence and existing evidence are immutable inputs", () => {
  ...
  assert.match(persistence, /if \(initialHistorical\)[\s\S]*batch\.set\(db\.collection\("users"\)\.doc\(account\.uid\)/); // initial cadence/backfill state is atomic with first Desk
  const route = read("src/app/api/boardsignal/[username]/route.ts");
  assert.match(route, /historicalPeriodStart:\s*historical\.periodStart/);
});
...
test("11-13 sparse activity: empty slots are evaluated but never fabricated", () => {
  ...
  assert.equal(fixtures.B.active.length, 2); // 13
...
test("18-23 retention: four imported Reviews equal one new-player activation, never four returns", () => {
  ...
  const establishedPlayerBackfill = summarizeOperationalRetention([
    { playerKey: "C", periodStart: "2026-08-13", periodEnd: "2026-08-19", source: "live" },
  ], [], []);
  assert.equal(establishedPlayerBackfill.r2Plus, 0);
  ...
  assert.equal(reviewCountsTowardRetention({ desk: { source: "live", provenance: { verified: true } } }), true);
  const founder = read("src/lib/boardsignal/server/founderOperations.ts");
  assert.match(founder, /period\.source !== "historical"/); // 23
  assert.match(founder, /reviewHistoryBackfill\?\.activationBaseline === true/);
  assert.match(read("src/lib/boardsignal/server/persistence.ts"), /activationBaseline:\s*true/);
});
...
test("38-41 failure/API: backfill is retryable and Chess.com archive access is cached/serial", () => {
  ...
  assert.doesNotMatch(worker, /25_000|60_000/);
  assert.match(worker, /embedded/);
  ...
  assert.match(processor, /historicalPeriodStart\?: string/);
...
test("42-52 regression surfaces remain wired", () => {
  ...
  assert.match(universalDesk, /stockfish/i); // 43
  ...
  assert.match(ask, /loadPublishedDesks/); // 48
...
test("G.3 Player Room integration stays presentation-safe", () => {
  ...
  assert.match(worker, /hidden aria-hidden="true"/);
  assert.match(worker, /embedded/);
});
