  ...
  return <main id="main" className={`beta-preview-room ${canContinue ? "is-approved" : ""}`}>
    <section className="container beta-preview-hero bs-surface-dark">
      <div className="beta-preview-identity">
        ...
        <div><p className="kicker">BOARD SIGNAL PREVIEW</p><h1>{approved ? "You're in." : `We found you, ${status?.canonicalUsername}.`}</h1><p>{approved ? "Your private BoardSignal Player Room is ready." : "Your games are here. BoardSignal can open your private Founding Beta Player Room now while identity review happens quietly in the background."}</p></div>
      </div>
      <div className="beta-preview-status"><span>{approved ? "IDENTITY CONFIRMED" : expired ? "ACCESS EXPIRED" : rejected ? "REQUEST CLOSED" : "PREVIEW READY"}</span><strong>{approved ? "PRIVATE PLAYER ROOM READY" : expired ? "FRESH LINK REQUIRED" : rejected ? "FOUNDER REVIEW CLOSED" : "FULL PRIVATE REVIEW AVAILABLE"}</strong>{status?.approvedAt ? <small>Founder reviewed {formatSync(status.approvedAt)}</small> : preview ? <small>Preview saved {formatSync(preview.generatedAt)}</small> : null}</div>
      {canContinue ? <button type="button" className="button button-lime beta-preview-primary-cta" onClick={openPlayerRoom} disabled={claiming}>{claiming ? <><LoaderCircle className="button-spinner" size={16}/> Opening</> : <>Continue to My Player Room <ArrowRight size={17}/></>}</button> : null}
    </section>

    {error ? <div className="container notice notice-error" role="alert">{error}</div> : null}

    {!preview ? <section className="container beta-preview-failure bs-surface-paper"><Sparkles/><div><p className="kicker">REQUEST SAVED</p><h2>Chess.com didn't return the preview yet.</h2><p>Your Founding Beta request is safe. Try the Preview again; identity review is not the gate to private access.</p></div><button type="button" className="button button-dark" onClick={retryPreview} disabled={retrying}>{retrying ? <><LoaderCircle className="button-spinner" size={15}/> Trying</> : <><RefreshCcw size={15}/> Try preview again</>}</button></section> : <PreviewContent preview={preview} ask={ask} showAskIntro={showAskIntro} accessCta={canContinue ? <section className="container beta-preview-next bs-surface-dark"><p className="kicker">READY FOR THE FULL PICTURE?</p><h2>Continue to your private Player Room now.</h2><p>Founding Beta access starts immediately. Identity checks happen quietly in the background.</p><button type="button" className="button button-lime" onClick={openPlayerRoom} disabled={claiming}>{claiming ? "Opening…" : "Continue to My Player Room"}<ArrowRight size={16}/></button></section> : null} returnChoice={status?.state === "preview_ready" ? <PreviewReturnChoice requestId={requestId} statusToken={statusToken} status={status} onStatus={setStatus} onError={setError} /> : null} />}
    ...
    {canContinue ? <div className="beta-preview-sticky-access"><button className="button button-lime" type="button" onClick={openPlayerRoom} disabled={claiming}>{claiming ? "Opening…" : "Continue to My Player Room"}<ArrowRight size={16}/></button></div> : null}
  </main>;
}
...
function PreviewReturnChoice({ requestId, statusToken, status, onStatus, onError }: { requestId: string; statusToken: string; status: BetaPreviewStatus; onStatus: (value: BetaPreviewStatus) => void; onError: (value: string) => void }) {
  ...
  return <section className="container beta-preview-return bs-surface-paper">
    <div className="beta-preview-section-heading"><div><p className="kicker">KEEP MY BOARDSIGNAL READY</p><h2>Optional: how should BoardSignal help you return later?</h2><p>Private access is already available above. Device alerts and external contact are optional return/recovery channels.</p></div>{savedLabel ? <span className="state-pill ready">{savedLabel}</span> : null}</div>
...
function PreviewContent({ preview, ask, showAskIntro, accessCta, returnChoice }: { preview: BoardSignalBetaPreview; ask: (prompt: string) => void; showAskIntro: boolean; accessCta?: ReactNode; returnChoice?: ReactNode }) {
  ...
  return <>
    ...
    <section className="container beta-preview-universe bs-surface-paper">
      <div className="beta-preview-section-heading"><div><p className="kicker">YOUR UNIVERSE PREVIEW</p><h2>If you entered the field today…</h2><p>Preview is provisional. Official public participation waits for Founder identity review or future Chess.com verification.</p></div><span className="state-pill processing">PROVISIONAL</span></div>
