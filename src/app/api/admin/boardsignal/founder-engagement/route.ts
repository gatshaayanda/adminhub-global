...
function currentChessForFounder(account: FounderSafeAccount) {
  const current = founderChessSnapshot(account.currentEpisodeSummary);
  const collection = account.currentEpisodeCollection;
  if (collection?.status === "retry_required") return undefined;
  if (collection?.status === "ok" && collection.periodStart && current?.periodStart !== collection.periodStart) return undefined;
  return current;
}
...
function chessSourceHealth(account: FounderSafeAccount, current?: FounderChessSnapshot): { status: ChessSourceHealth; checkedAt?: string } {
  const collection = account.currentEpisodeCollection;
  if (collection?.status === "retry_required") return { status: "retry_required", checkedAt: collection.checkedAt };
  if (!current) return { status: "not_checked", checkedAt: collection?.checkedAt };
  return { status: current.games === 0 ? "zero_games" : "ok", checkedAt: collection?.checkedAt ?? current.checkedAt };
}

function reviewStatus(account: FounderSafeAccount, summary?: FounderPlayerSummary): ReviewStatus | undefined {
  if (summary?.row.reviewCheckRequired) return "CHECK REQUIRED";
  if (summary?.row.readyNotSeen) return "READY";
  if (account.currentEpisodeSummary?.status === "forming" || summary?.row.forming) return "FORMING";
  if (summary?.row.latestReportPeriod?.outcome === "no_activity") return "NO ACTIVITY";
  if (summary?.row.latestReview) return "COMPLETED";
  return undefined;
}
  ...
  return {
    ...
    currentBoardSignal: {
      ...
      coaching: coaching ? {
        ...
        needsReview: Boolean(coaching.ladderExhaustedAt),
      } : undefined,
    },
    chess: {
      current,
    ...
    review: {
      status: reviewStatus(account, summary),
      daysComplete: current ? account.currentEpisodeSummary?.daysComplete : undefined,
      daysRemaining: current ? account.currentEpisodeSummary?.daysRemaining : undefined,
      dueAt: summary?.row.nextDeskDueAt ?? account.currentEpisodeSummary?.nextDeskDueAt ?? account.nextDeskDueAt,
      completedCount: summary ? count(summary.validation.verifiedReviews) : undefined,
      latest: latestReview ? {
        periodStart: latestReview.periodStart,
        periodEnd: latestReview.periodEnd,
        periodLabel: latestReview.periodLabel,
      } : undefined,
    },
    trustpilot: {
...
export async function GET(request: Request) {
  try {
    ...
    const attention = record(aggregate.attention);
    const validation = record(aggregate.validation);
    const engagement = record(aggregate.engagement);
    const coaching = record(aggregate.coaching);
    let rows: ReturnType<typeof playerRow>[] = [];

    if (includeRows) {
      const [players, summaries] = await Promise.all([
        db.collection("users").where("role", "==", "player").get(),
        db.collection("founderPlayerSummaries").where("active", "==", true).get(),
      ]);
      const summaryByUid = new Map(summaries.docs.map((document) => {
      ...
      rows = players.docs
        ...
        .map((account) => playerRow(account, summaryByUid.get(account.uid)))
    ...
    return response({
      ...
      intelligence: {
        ...
        coaching: {
          totalReactions: count(coaching.totalReactions),
          helpful: count(coaching.helpful),
          notHelpful: count(coaching.notHelpful),
          l1DownToL2: count(coaching.l1DownToL2),
          l2DownToL3: count(coaching.l2DownToL3),
          level3Reached: count(coaching.level3Reached),
          level3Down: count(coaching.level3Down),
          viewGame: count(coaching.viewGame),
          exampleCycles: count(coaching.exampleCycles),
          askEscalations: count(coaching.askEscalations),
          byFamily: record(coaching.byFamily),
          byLevel: record(coaching.byLevel),
        },
        validation: {
